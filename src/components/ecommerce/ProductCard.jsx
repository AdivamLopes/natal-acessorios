    ...
    >
      <Link to={`/produto/${product.id}`} className="flex flex-col flex-grow">
        <CardHeader className="p-0 relative overflow-hidden aspect-[4/5]">
          ...
          <Button variant="ghost" size="icon" onClick={handleWishlist} className="absolute top-3 right-3 h-8 w-8 bg-white/80 rounded-full hover:bg-white transition-colors opacity-0 group-hover:opacity-100">
            <Heart className="h-4 w-4 text-gray-600 hover:text-carmine" />
          </Button>
          ...
          <div className="absolute bottom-3 left-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity transform group-hover:translate-y-0 translate-y-4 duration-300">
            <Button onClick={handleAddToCart} size="sm" className="w-full bg-white/90 text-carmine hover:bg-white backdrop-blur-sm font-semibold">
              <ShoppingBag className="h-4 w-4 mr-2" />
        ...
        <CardContent className="p-4 flex-grow">
          <h3 className="text-lg font-semibold text-gray-800 mb-1 hover:text-carmine transition-colors truncate">{product.name}</h3>
