...
function extractHeadData(content, filePath, routes) {
    const cleanedContent = cleanContent(content);

    if (!EXTRACTION_REGEX.headTest.test(cleanedContent)) {
        return null;
    }

    const headMatch = content.match(EXTRACTION_REGEX.head);
    if (!headMatch) return null;

    const headContent = headMatch[1];
    const titleMatch = headContent.match(EXTRACTION_REGEX.title);
    const descMatch = headContent.match(EXTRACTION_REGEX.description);
...
function processPageFile(filePath, routes) {
    try {
        ...
        return extractHeadData(content, filePath, routes);
    } catch (error) {
...
function main() {
    ...
    if (pages.length === 0) {
        console.error('❌ No pages with <Head> components found!');
        process.exit(1);
    }
